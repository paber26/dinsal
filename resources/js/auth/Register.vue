...
<div class="w-full bg-white rounded-xl p-2.5">
    <div class="border-2 border-gray-300 p-4 rounded-xl">
        <form @submit.prevent="register()" class="">
            <!-- {{ csrf_token() }} -->
            <label class="text-birumateri font-semibold text-base">Username</label>
            <input v-model="user.username" type="text" class="w-full p-1.5 bg-white border-2 border-birumateri rounded-md focus:outline-none focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <label class="text-birumateri font-semibold text-base mt-3">Tanggal Lahir</label>
            <input v-model="user.tanggal_lahir" type="date" class="w-full p-1.5 bg-white border-2 border-birumateri rounded-md focus:outline-none focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
    ...
    methods: {
        register() {
            ...
            if (this.user.username == '') {
                this.$swal('Masukkan username kamu')
                return
            }
            if (this.user.tanggal_lahir == '') {
                this.$swal('Masukkan tanggal lahir kamu')
                return
            }
            ...
            axios.post('/register', this.user).then(response => {
                if (response.data == 'Sudah terdaftar') {
                    this.$swal('Username sudah terdaftar')
